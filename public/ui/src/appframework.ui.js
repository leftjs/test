...
(function($) {"use strict";
	...
	var ui = function() {
		...
		window.addEventListener("popstate", function() {
			...
			if (id === "")
				return;
			if (af(id).filter(".panel").length === 0)
				return;
	...
	ui.prototype = {
		...
		disableNativeScrolling : function() {
			af.feat.nativeTouchScroll = false;
		},
		...
		updateHash : function(newHash) {
			...
			//force having the # in the begginning as a standard
			previousTarget = newHash;

			var previousHash = window.location.hash;
			var panelName = this.getPanelId(newHash).substring(1);
			//remove the #
			try {
		...
		setTitle : function(val) {
			if (this._currentHeaderID !== "defaultHeader")
				return;
		...
		setBackButtonText : function(text) {
			...
			if (this.backButtonText.length > 0)
				...
				$.query("#header header:not(.ignore)  #backButton").html(text);
		},
		/**
		...
		setBackButtonVisibility : function(show) {
			if (!show)
				...
				$.query("#header #backButton").css("visibility", "visible");
		},
		/**
		...
		updatePanel : function(id, content) {
			...
			if (newDiv.children('.panel') && newDiv.children('.panel').length > 0)
				newDiv = newDiv.children('.panel').get(0);
			else
				newDiv = newDiv.get(0);
		...
		addContentDiv : function(el, content, title, refresh, refreshFunc) {
			...
			if (!myEl) {
				...
				if (!newDiv.getAttribute("data-title") && title)
					newDiv.setAttribute("data-title", title);
				newId = (newDiv.id) ? newDiv.id : el.replace("#", "");
				//figure out the new id - either the id from the loaded div.panel or the crc32 hash
				newDiv.id = newId;
				if (newDiv.id != el)
		...
		addDivAndScroll : function(tmp, refreshPull, refreshFunc, container) {
			...
			if (!$.feat.nativeTouchScroll && hasScroll)
				tmp.setAttribute("js-scrolling", "true");
		...
		loadContent : function(target, newTab, back, transition, anchor) {
			...
			//Hack to allow passing in no anchor
			if (target.indexOf("#") == -1) {
				var urlHash = "url" + crc32(target);
				//Ajax urls
		...
		loadContentData : function(what, newTab, back, transition) {
			...
			if (back) {
				if (this.history.length > 0) {
					...
					if (el)
						this.setBackButtonText(el.getAttribute("data-title"));
					else
			...
			} else if (this.showBackButton && this.showBackbutton)
		...
		loadAjax : function(target, newTab, back, transition, anchor) {
			// XML Request
			if (this.activeDiv.id == "afui_ajax" && target == this.ajaxUrl)
				return;
			var urlHash = "url" + crc32(target);
			//Ajax urls
			var that = this;
			if (target.indexOf("http") == -1)
				target = intel.xdk.webRoot + target;
		...
		launch : function() {
			...
			if (!this.menu) {
				...
				if ($.feat.nativeTouchScroll)
					$.query("#menu_scroller").css("height", "100%");
			...
			if (this.firstDiv) {
				...
				var loadFirstDiv = function() {
					...
					if (firstPanelId.length > 0 && that.loadDefaultHash && !isFirstPanel) {
						that.loadContent(defaultHash, true, false, 'none');
						//load the active page as a newTab with no transition
					} else {
	...
	var checkAnchorClick = function(e, theTarget) {
		...
		//let's try the parent (recursive)
		//anchors
		if (theTarget.tagName !== "undefined" && theTarget.tagName.toLowerCase() == "a") {
			...
			if (custom !== false) {
				if ($.ui.customClickHandler(theTarget, e))
			...
			var href = theTarget.href, prefix = location.protocol + "//" + location.hostname + ":" + location.port + location.pathname;
